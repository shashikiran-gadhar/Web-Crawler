...
def main_crawler(seed,key):
    ...
    while len(MS) < 1000:
        # Get URL that needs to be crawled using BFS Algo
        pg_url = get_links(D1)
        ...
        if pg_url == "No links":
            ...
            if pg_url in D1:
                D1_urls = pg_crawler(pg_url,key)
                for x in D1_urls:
                    D2.append(x)
            elif pg_url in D2:
                D2_urls = pg_crawler(pg_url, key)
                for y in D2_urls:
                    if (y not in D1) and (y not in D2):
                        D3.append(y)
            elif pg_url in D3:
                D3_urls = pg_crawler(pg_url, key)
                for z in D3_urls:
                    if (z not in D2) and (z not in D3):
                        D4.append(z)
            elif pg_url in D4:
                D4_urls = pg_crawler(pg_url, key)
                for x in D4_urls:
